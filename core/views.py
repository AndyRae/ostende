...
class ScreeningDetailView(DetailView, MultipleObjectMixin):
    ...
    def get_context_data(self, **kwargs):
        todaysdate = datetime.now().date()
        object_list = Screening.objects.filter(venue=self.object.venue).filter(date__gte=todaysdate).order_by('date')[:3]
...
class ArticleDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
	        ...
	        todaysdate = datetime.now().date()
	        # Add in a QuerySet for all objects
	        context['articles'] = Article.objects.filter(date__lte=todaysdate).order_by('?')[:3]
	        return context
