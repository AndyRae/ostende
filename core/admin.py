...
class ScreeningResource(resources.ModelResource):
    film = fields.Field(
        column_name="film",
        attribute="film",
        widget=ForeignKeyWidget(Film, field="name"),
    )
    venue = fields.Field(
        column_name="venue",
        attribute="venue",
        widget=ForeignKeyWidget(Venue, field="name"),
    )
    season = fields.Field(
        column_name="season",
        attribute="season",
        widget=ForeignKeyWidget(Season, field="name"),
    )
    programme = fields.Field(
        column_name="programme",
        attribute="programme",
        widget=ForeignKeyWidget(Programme, field="name"),
    )

    class Meta:
        film = fields.Field(
            column_name="film", attribute="film", widget=ForeignKeyWidget(Film, "name")
        )
        model = Screening
        fields = (
            "id",
